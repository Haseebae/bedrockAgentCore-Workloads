...
log_ext = os.getenv("AGG_LOG_EXT", "2026-03-12")
        ...
        for query_key, trace_data in traces.items():
            ...
            for state in states:
                ...
                if not if_checkpointer and memory_val.lower() in ["e", "n", "empty", "naive"]:
                    print("Setting memory_cents to 0 for E/N memory configs")
                    memory_cents = 0.0
...
def main():
    ...
    parser.add_argument("--if_checkpointer", default="false", help="If false, sets memory_events_cents to 0 for E/N memory configs. if true, computed for all")
    ...
    if_checkpointer = args.if_checkpointer == "true"
    print(f"Aggregating batches with if_checkpointer: {if_checkpointer}")
    
    for batch_name, run_paths in batches.items():
        if len(run_paths) > 0:
            aggregate_batch(batch_name, run_paths, if_checkpointer=if_checkpointer)
