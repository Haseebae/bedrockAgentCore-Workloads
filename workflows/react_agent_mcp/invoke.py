...
# ─── Configuration ──────────────────────────────────────────────────────────
REGION = "ap-south-1"
RUNTIME_ARN = "arn:aws:bedrock-agentcore:ap-south-1:235319806087:runtime/react_agent_mcp-x4nRto3n49"
...
if __name__ == "__main__":
    prompt = (
        " ".join(sys.argv[1:])
        if len(sys.argv) > 1
        else "Download the paper 'Attention Is All You Need' and summarize its key contributions."
    )
